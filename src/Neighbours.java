...
public class Neighbours extends Application {
	...
	public void init() {
		...
		for (int i = 0; i < dist.length; i++) {
			arr[i] = (int) Math.round(dist[i]*nLocations);
		}
...
//	Actor[][] shuffel(Actor[][] arr) {
//		
//		Actor[] a = ArrTo1DArr(arr);
//		shuffel(a);
//		return arrTo2DArr(a, arr.length);
//		
//	}	
	
	void shuffel(Actor[] arr) {
	...
	Actor chooseColor(double[] dist, int index){
		...
		case 2:
			return Actor.RED;
		case 3:
			return Actor.NONE; //Can add more colors
		default:
			return Actor.NONE;
		}		
	}

	Actor[] ArrTo1DArr(Actor[][] actor) {
	...
	Actor[][] arrTo2DArr(Actor[] arr, int arrDim){
		Actor[][] a = new Actor[arrDim][arrDim];
